...
let TransportDashboardComponent = class TransportDashboardComponent {
    ...
    menu = [
        ...
        { label: 'Recettes & Versements', icon: '▣' },
        { label: 'Transactions', icon: '▤' },
        { label: 'Itinéraires', icon: '♧' },
        { label: 'Rapports', icon: '▥' },
    ...
    revenueHistory = [670, 610, 760, 940, 660, 790, 930, 1110, 790, 810, 830, 920, 890, 1000, 1190, 950, 790, 860, 920, 790, 940, 1160, 1000, 1150, 940, 1250, 1130, 1330, 1300];
    chartLabels = ['13 Mai', '16 Mai', '19 Mai', '22 Mai', '25 Mai', '28 Mai', '31 Mai', '03 Juin', '06 Juin', '09 Juin', '11 Juin'];
    chartPoints = '';
    chartArea = '';
    chartDots = [];
    ngOnInit() { this.buildChart(); }
    get filteredBuses() {
        return this.buses.filter(bus => this.selectedBus === 'all' || bus.plate === this.selectedBus);
    }
    get filteredPayments() {
        return this.payments.filter(payment => this.selectedBus === 'all' || payment.bus === this.selectedBus);
    }
    get activeBuses() { return this.buses.filter(bus => bus.status === 'En service').length; }
    get uniqueBusPlates() { return [...new Set(this.buses.map(bus => bus.plate))]; }
    get selectedPeriodLabel() { return this.period === 'today' ? 'du jour' : this.period === 'week' ? 'de la semaine' : 'du mois'; }
    formatBif(amount) {
        return `${new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 0 }).format(amount)} BIF`;
    }
    formatNumber(amount) { return new Intl.NumberFormat('fr-FR').format(amount); }
    trackByIndex(index) { return index; }
    refresh() { this.lastRefresh = new Date(); this.visibleBusCount = 10; this.visiblePaymentCount = 10; }
    showAllBuses() { this.visibleBusCount = this.filteredBuses.length; }
    showAllPayments() { this.visiblePaymentCount = this.filteredPayments.length; }
    chooseMenu(label) { this.activeMenu = label; }
    viewAlert(alert) { this.selectedAlert = alert; }
    viewPayment(payment) { this.selectedPayment = payment; }
    closeDialog() { this.selectedAlert = null; this.selectedPayment = null; }
    buildChart() {
...
TransportDashboardComponent = __decorate([
    Component({
...
export { TransportDashboardComponent };
