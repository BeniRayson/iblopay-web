...
function genererHistorique() {
    ...
    for (var _i = 0, busConfig_1 = busConfig; _i < busConfig_1.length; _i++) {
        var cfg = busConfig_1[_i];
        for (var j = 0; j < 25; j++) {
            var passagers = 12 + Math.floor(Math.random() * 18);
            historique.push({
    ...
    for (var _a = 0, taxiConfig_1 = taxiConfig; _a < taxiConfig_1.length; _a++) {
        var cfg = taxiConfig_1[_a];
        for (var j = 0; j < 20; j++) {
            var distance = 3 + Math.floor(Math.random() * 12);
            historique.push({
    ...
    return historique.sort(function (a, b) { return b.date.getTime() - a.date.getTime(); });
}
export var MOCK_HISTORIQUE_TRAJETS = genererHistorique();
// ─── INCIDENTS ──────────────────────────────────────────────
export var MOCK_INCIDENTS_TRANSPORT = [
...
//# sourceMappingURL=mock-transport-data.js.map