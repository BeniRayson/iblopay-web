...
var AgentService = function () {
    ...
    var AgentService = _classThis = /** @class */ (function () {
        ...
        // MÉTHODES PUBLIQUES
        // ============================================
        AgentService_1.prototype.getAgents = function () {
            return of(__spreadArray([], this.mockAgents, true));
        };
        AgentService_1.prototype.getAgentById = function (id) {
            var agent = this.mockAgents.find(function (a) { return a.id === id; });
            if (agent) {
                return of(__assign({}, agent));
            }
            return throwError(function () { return new Error('Agent non trouvé'); });
        };
        AgentService_1.prototype.createAgent = function (agentData) {
            var newAgent = {
                id: Date.now().toString(),
                code: "SA-".concat(String(this.mockAgents.length + 1).padStart(3, '0')),
                firstName: agentData.firstName,
                lastName: agentData.lastName,
                email: agentData.email || "".concat(agentData.firstName.toLowerCase(), ".").concat(agentData.lastName.toLowerCase(), "@iblopay.com"),
                phone: agentData.phone,
                cin: agentData.cin,
                cardNumber: agentData.cardNumber || "CARTE-IBP-2024-".concat(String(this.mockAgents.length + 1).padStart(4, '0')),
                ...
                address: {
                    ...
                    completeAddress: "".concat(agentData.zone || agentData.quartier || agentData.colline, ", ").concat(agentData.commune, ", ").concat(agentData.province)
                },
                nif: agentData.nif,
        ...
        AgentService_1.prototype.updateAgent = function (id, agentData) {
            var index = this.mockAgents.findIndex(function (a) { return a.id === id; });
            if (index === -1) {
                return throwError(function () { return new Error('Agent non trouvé'); });
            }
            var currentAgent = this.mockAgents[index];
            if (!currentAgent) {
                return throwError(function () { return new Error('Agent non trouvé'); });
            }
            var updatedAgent = __assign(__assign(__assign({}, currentAgent), agentData), { updatedAt: new Date() });
            this.mockAgents[index] = updatedAgent;
            return of(updatedAgent);
        };
        AgentService_1.prototype.deleteAgent = function (id) {
            var index = this.mockAgents.findIndex(function (a) { return a.id === id; });
            if (index === -1) {
                return throwError(function () { return new Error('Agent non trouvé'); });
            }
        ...
        AgentService_1.prototype.getAgentStats = function () {
            var total = this.mockAgents.length;
            var active = this.mockAgents.filter(function (a) { return a.status === AgentStatus.ACTIVE; }).length;
            var pending = this.mockAgents.filter(function (a) { return a.status === AgentStatus.PENDING; }).length;
            var blocked = this.mockAgents.filter(function (a) { return a.status === AgentStatus.BLOCKED; }).length;
            var suspended = this.mockAgents.filter(function (a) { return a.status === AgentStatus.SUSPENDED; }).length;
            var inactive = this.mockAgents.filter(function (a) { return a.status === AgentStatus.INACTIVE; }).length;
            var totalElectronics = this.mockAgents.reduce(function (acc, a) { var _a; return acc + (((_a = a.electronics) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0);
            var totalAgents = this.mockAgents.reduce(function (acc, a) { var _a; return acc + (((_a = a.agents) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0);
            var totalDeposits = this.mockAgents.reduce(function (acc, a) { var _a; return acc + (((_a = a.deposits) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0);
            var totalDepositAmount = 0;
            var totalTransactionAmount = 0;
            this.mockAgents.forEach(function (agent) {
                var _a, _b;
                (_a = agent.deposits) === null || _a === void 0 ? void 0 : _a.forEach(function (d) {
                    if (d.status === 'COMPLETED') {
                        totalDepositAmount += d.amount;
                    }
                });
                (_b = agent.agents) === null || _b === void 0 ? void 0 : _b.forEach(function (subAgent) {
                    var _a;
                    (_a = subAgent.transactions) === null || _a === void 0 ? void 0 : _a.forEach(function (t) {
        ...
        AgentService_1.prototype.getSubAgents = function (agentId) {
            var agent = this.mockAgents.find(function (a) { return a.id === agentId; });
            return of((agent === null || agent === void 0 ? void 0 : agent.agents) || []);
        };
        AgentService_1.prototype.getElectronics = function (agentId) {
            var agent = this.mockAgents.find(function (a) { return a.id === agentId; });
            return of((agent === null || agent === void 0 ? void 0 : agent.electronics) || []);
        };
        AgentService_1.prototype.getDeposits = function (agentId) {
            var agent = this.mockAgents.find(function (a) { return a.id === agentId; });
            return of((agent === null || agent === void 0 ? void 0 : agent.deposits) || []);
        };
        AgentService_1.prototype.searchAgents = function (query) {
            if (!query || query.trim() === '') {
                return of(__spreadArray([], this.mockAgents, true));
            }
            var searchTerm = query.toLowerCase().trim();
            var filtered = this.mockAgents.filter(function (a) {
                return a.firstName.toLowerCase().includes(searchTerm) ||
